...
uninstall() {
    ...
    systemctl stop bunny-v2ray
    systemctl disable bunny-v2ray
    rm /etc/systemd/system/bunny-v2ray.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/bunny-v2ray/ -rf
    rm /usr/local/bunny-v2ray/ -rf

    echo ""
    echo -e "The uninstallation is successful. If you want to delete this script, exit the script and run ${green}rm /usr/bin/bunny-v2ray -f${plain} Delete"
    echo ""
...
reset_user() {
    ...
    /usr/local/bunny-v2ray/bunny-v2ray setting -username admin -password admin
...
reset_config() {
    ...
    /usr/local/bunny-v2ray/bunny-v2ray setting -reset
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/bunny-v2ray/bunny-v2ray setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}bunny-v2ray started successfully${plain}"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            echo -e "${green}bunny-v2ray and xray stopped successfully${plain}"
        else
...
restart() {
    systemctl restart bunny-v2ray
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}bunny-v2ray and xray restart successfully${plain}"
    else
...
status() {
    systemctl status bunny-v2ray -l
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}bunny-v2ray is set to start successfully after booting${plain}"
    else
        echo -e "${red}bunny-v2ray settings fail to start automatically after booting${plain}"
    fi
...
disable() {
    systemctl disable bunny-v2ray
    if [[ $? == 0 ]]; then
        echo -e "${green}bunny-v2ray cancels the boot and self-start successfully${plain}"
    else
        echo -e "${red}bunny-v2ray cancels the boot-up auto-start failure${plain}"
    fi
...
show_log() {
    journalctl -u bunny-v2ray.service -e --no-pager -f
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
migrate_v2_ui() {
    /usr/local/bunny-v2ray/bunny-v2ray v2-ui
...
update_shell() {
    wget -O /usr/bin/bunny-v2ray -N --no-check-certificate https://github.com/BunnyDevelopers/Bunny-V2Ray-Manager-Bot/raw/master/bunny-v2ray.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/bunny-v2ray.service ]]; then
        return 2
    fi
    temp=$(systemctl status bunny-v2ray | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled bunny-v2ray)
...
show_usage() {
    clear
    echo "Bunny-V2ray-Manager management script usage method: "
    echo "------------------------------------------"
    echo "Bunny-V2ray-Manager start        - Launch Bunny-V2ray-Manager bot"
    echo "Bunny-V2ray-Manager stop         - Stop Bunny-V2ray-Manager bot"
    echo "Bunny-V2ray-Manager restart      - Restart the Bunny-V2ray-Manager bot"
    echo "Bunny-V2ray-Manager update       - Update Bunny-V2ray-Manager bot"
    echo "Bunny-V2ray-Manager install      - Install the Bunny-V2ray-Manager bot"
    echo "Bunny-V2ray-Manager uninstall    - Uninstall the Bunny-V2ray-Manager bot"
    echo "------------------------------------------"
}
...
show_menu() {
    ...
    echo -e "\E[44;1;37m               🐰 BUNNY V2RAY MANAGER 🐰              \E[0m"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    echo -e "\033[0;34m  #BY:      🥕🥕️🥕️ BUNNY DEVELOPERS 🥕️🥕️🥕       \033[0m"
    echo -e "\033[0;34m                     SRI LANKA                    \033[0m"
    echo -e "\033[0;34m                Telegram- @BUNNY_DEVELOPERS               \033[0m"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    show_status
    echo -e "
  ...
  [\033[1;36m2\033[1;31m] \033[1;37m• \033[1;33mUPDATE BUNNY-V2RAY \033[1;31m     [\033[1;36m6\033[1;31m] \033[1;37m• \033[1;33mSTOP BOT \033[1;31m
